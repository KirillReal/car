...
function drawAds(ads) {
    ...
    ads.forEach(function(item, i, arr) {
        ...
        if (item.car.carPhotos.length === 0) {
            rsl +=          '<img src="/photos/noimage2.jpeg" width="212">';
        } else {
            rsl +=          '<img src="ads/photo?name=' + item.car.carPhotos[0].id + '" width="212">';
        }
        rsl +=          '</p>' +
            '<div class="auto-description">' +
            '<p class="auto-link"><a href="ads?page=view&id=' + item.id + '">' + item.car.carModel.name + '</a></p>' +
